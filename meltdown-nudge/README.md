...
| `noise.level` | `0.16` | 音量門檻 0～1（RMS）。0.10 很容易跳／0.16 一般／0.24 要崩潰大哭 |
| `noise.sustainMs` | `2500` | 要吵夠這麼久才跳 |
...
用一秒 150ms 的取樣、`sustainMs 2500`、`decay 0.35` 跑出來的：

| 情況 | 結果 |
|---|---|
| 一直大哭 | 2.4 秒跳 |
| 哭 2 秒停 1 秒（換氣） | 3.6 秒跳 |
| 哭 1 秒停 1.5 秒（斷斷續續） | 10.5 秒跳 |
| 只喊一聲 | 不跳 |
| 電視背景音（低於門檻） | 不跳 |
