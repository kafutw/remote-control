...
2. 設定值存在這個 App 自己的 localStorage 紀錄裡，欄位建議：`noiseAt`（0 / 0.05 / 0.08 / 0.12 / 0.20）、`nudgeAt`（0 / 3 / 5）、`tone`（`auto` / `soft` / `fun`）。**預設 `noiseAt: 0.08`、`nudgeAt: 0`**（只有崩潰大叫才跳，答錯不打斷）。門檻要低一點才抓得到——手機麥克風收到的哭聲 RMS 常常只有 0.09 左右。
